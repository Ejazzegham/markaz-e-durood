...
- All data lives in Firestore, via a thin adapter (`src/lib/db/collection.ts` and `src/lib/db/firestore.ts`) that keeps the same `findMany`/`create`/`update`/`delete` shape the admin panel already used. Collections: `users`, `duroodSubmissions`, `donations`, `bookResources`, `naats`, `bayans`, `galleryImages`, `audioResources`, `videoResources`, `blogPosts`, `news`, `faqs`.
- Firestore will prompt (with a direct link in the server logs) to create a composite index the first time a query needs one — e.g. the rapid-submission guard on the Durood counter and Donations (`ipAddress` + `createdAt`), or a member's own history on the dashboard (`userId` + `createdAt`). Click that link once and it's a one-time setup per query shape.
- Both Durood submissions and donations can be sent by anyone — no account required. If the visitor is signed in, the submission is also linked to their account so it shows up in their `/dashboard` history; guests just submit under a typed name or anonymously. Donations are recorded as a pledge/request (there's no payment gateway wired up — no Stripe/PayPal keys in `.env`), the same as the original "your donation request has been received" placeholder implied; collecting the actual funds still happens outside this flow.
