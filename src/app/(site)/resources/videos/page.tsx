...
import { FaVideo, FaArrowLeft, FaFilm, FaHeart } from 'react-icons/fa'
...
export default function Videos() {
  const [videos, setVideos] = useState<PlayableItem[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    fetch('/api/content/video')
      .then((res) => res.json())
      .then((data) => {
        const items: PlayableItem[] = (data.items || []).map((v: any) => ({
          id: v.id,
          title: v.title,
          subtitle: v.category,
    ...
    <div className="min-h-screen bg-gradient-to-b from-green-950 via-green-850 to-green-950 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 px-6 md:px-8 py-4">
                ...
                <div className="flex items-center gap-3">
                  <div className="relative">
                    ...
                    <div className="relative p-2.5 rounded-xl bg-gradient-to-br from-gold-500/30 to-gold-500/10 border border-gold-500/30">
                      <FaVideo className="text-gold-500 text-xl" />
                    </div>
                  </div>
                  <div>
                    ...
                    <p className="text-gray-400 text-xs hidden sm:block">
                      Watch Islamic videos, Naat performances and educational content
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-gold-500/10 border border-gold-500/20">
                  <FaFilm className="text-gold-500 text-xs" />
                  <span className="text-gray-300 text-xs font-medium">{videos.length} Videos</span>
                </div>
        ...
        <YoutubeStyleTheater
          items={videos}
          loading={loading}
          emptyMessage="No videos have been added yet. Check back soon!"
          kindLabel="Videos"
          subtitleFieldLabel="Category"
          accentIcon={<FaFilm className="inline text-gold-500 text-xs" />}
        />
        ...
        <div className="mt-4 text-center">
          <p className="text-[10px] text-gray-600 tracking-wider">
            <FaVideo className="inline mr-1.5 text-gold-500/30 text-[10px]" />
            Bringing Islamic knowledge to life through video
          </p>
        </div>
