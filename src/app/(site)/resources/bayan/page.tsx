...
export default function Bayan() {
  const [allBayan, setAllBayan] = useState<PlayableItem[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    fetch('/api/content/bayan')
      .then((res) => res.json())
      .then((data) => {
        const items: PlayableItem[] = (data.items || []).map((b: any) => ({
          id: b.id,
          title: b.title,
          subtitle: b.speaker,
    ...
    <div className="min-h-screen bg-gradient-to-b from-green-950 via-green-850 to-green-950 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 px-6 md:px-8 py-4">
              ...
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-gold-500/10 border border-gold-500/20">
                  <FaMicrophone className="text-gold-500 text-xs" />
        ...
        <YoutubeStyleTheater
          items={allBayan}
          loading={loading}
          emptyMessage="No bayan have been added yet. Check back soon!"
          kindLabel="Bayan"
          subtitleFieldLabel="Speaker"
